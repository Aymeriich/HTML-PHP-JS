<?php
    ...
    if (isset($_SESSION["user"])) {
        ...
        $sql = "SELECT nom, cognoms FROM clients WHERE id_client = '$usuari'";
        $result = $conn->query($sql);
        
        if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			$entrar = "class=\"my-0 mx-2\">$row[nom] $row[cognoms]";
            $nou_client = "href=\"tancar.php\" class=\"btn btn-outline-primary my-0\">Tancar sessió";
        }
    } else {
        $entrar = "class=\"btn btn-primary my-0 mx-2\">Entrar";
        $nou_client = "href=\"form-client.php\" class=\"btn btn-outline-primary my-0\">Nou client";
    }
?>
	...
	<body class="bg-primary">
		<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
			...
			<div class="collapse navbar-collapse" id="navbarCollapse">
				...
				<a href="entrar.php" <?php echo $entrar ?></a>
				<a <?php echo $nou_client ?></a>
			</div>
		</nav>